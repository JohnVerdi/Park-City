...
(function($,window, document){
	...
	$(function(){
		...
		$(window).load(function(){
			...
			$(window).scroll(function()
			{
				if($(document).scrollTop() > _headerHeight)
				{
					...
					$('.wpl-28-header-sticky body').css("margin-top", _headerHeight);
				}
				else
				{
					$('.wpl-28-header-sticky #header').removeClass('sticky');
					$('.wpl-28-header-sticky body').css("margin-top", "0");
				}
			});
		...
		if($(window).width() < 992)
		{
			$(".wpl-28-resp-menu .header_lang_unit_switcher").append($("#header .header_lang_unit_switcher .widget"));
		}
