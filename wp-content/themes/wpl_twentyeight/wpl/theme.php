...
class wpl_theme
{
    ...
    */
    public function wpl_social_icon($id)
    {
        ...
        if(isset($this->theme_options['gplus']) and trim($this->theme_options['gplus'])!==''):
            echo '<li><a href="'.$this->theme_options['gplus'].'" class="g_plus" target="_blank">'.__('Google+', 'wplt').'</a></li>';
        endif;
    ...
    */
    public function wpl_top_logo()
    {
		...
		echo ((isset($this->theme_options['retina_logo']) and trim($this->theme_options['retina_logo']) !=='') ? '<img itemprop="logo" src="'.$this->theme_options['retina_logo'].'" id="retina_top_logo" alt="'.__(get_bloginfo( 'name' ), 'wplt').'" />' : '');
		echo '</a>';
    }
